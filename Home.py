...
df_selection = df.query(
    "Anno == @anno & Mese == @mese & UM==@um & Fornitori==@fornitori"
)
...
Lista_Gruppo3_Punti= (
    df_selection.groupby(by=["Gruppo3"]).sum()[["Punti"]].sort_values(by="Gruppo3")
)
Lista_Gruppo3_Qta= (
    df_selection.groupby(by=["Gruppo3"]).sum()[["Quantita"]].sort_values(by="Gruppo3")
)


# SALES( BY PRODUCT LINE [BAR CHART]

fig_Gruppo3_qta = px.bar(
    Lista_Gruppo3_Qta,
    y="Quantita",
    x=Lista_Gruppo3_Qta.index,
    orientation="v",
    title="<b>Paniere</b>",
    color_discrete_sequence=["#0083B8"] * len(Lista_Gruppo3_Qta),
    template="plotly_white",
    labels={'Classi di Prodotto': 'Numero'}, height=400,
)
fig_Gruppo3_qta.update_layout(
...
right_column.write(Lista_Gruppo3_Qta)
left_column.plotly_chart(fig_Gruppo3_qta, use_container_width=True)

st.markdown("""---""")

fig_Gruppo3_punti = px.bar(
    Lista_Gruppo3_Punti,
    y="Punti",
    x=Lista_Gruppo3_Punti.index,
    orientation="v",
    title="<b>Paniere</b>",
    color_discrete_sequence=["#0083B8"] * len(Lista_Gruppo3_Punti),
    template="plotly_white",
    labels={'Classi di Prodotto': 'Numero'}, height=400, width=1600,

)
fig_Gruppo3_punti.update_layout(
    plot_bgcolor="rgba(0,0,0,0)",
    xaxis=(dict(showgrid=False))
)

left_column, right_column = st.columns([400,150])
right_column.write(Lista_Gruppo3_Punti)
left_column.plotly_chart(fig_Gruppo3_punti, use_container_width=True)
